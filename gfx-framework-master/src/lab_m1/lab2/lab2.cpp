...
void Lab2::Init()
{
    ...
    {
        vector<VertexFormat> tetrahedronVertices
        {
        ...
        vector<unsigned int> tetrahedronIndices =
        {
            /*     0, 1, 2,
                 0, 2, 3,
                 0, 3, 1,
...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    RenderMesh(meshes["tetrahedron"], shaders["VertexColor"], glm::vec3(0, 0.5f, 2), glm::vec3(0.5f));

    // TODO(student): Draw the square
    RenderMesh(meshes["square"], shaders["VertexColor"], glm::vec3(1, 0.5f, -1.5f), glm::vec3(0.5f));
