...
void Lab5::Update(float deltaTimeSeconds)
{
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(2.5f, 1.5f, -3));
...

    {
        ...
        modelMatrix = glm::rotate(modelMatrix, RADIANS(90.0f), glm::vec3(0, 1, 1));
        modelMatrix = glm::scale(modelMatrix, glm::vec3(1.5f, 0.5f, 1.0f));
        RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix);
    }
...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (isPerspective) {
        float fovChangeSpeed = RADIANS(10) * deltaTime;
    ...
    if (!isPerspective) {
        float orthoChangeSpeed = 1.0f * deltaTime;

        if (window->KeyHold(GLFW_KEY_I)) {
            orthoHeight += orthoChangeSpeed;
        }

        if (window->KeyHold(GLFW_KEY_K)) {
            orthoHeight = glm::max(orthoHeight - orthoChangeSpeed, 0.1f);
        }

        if (window->KeyHold(GLFW_KEY_J)) {
            orthoWidth += orthoChangeSpeed;
        }

        if (window->KeyHold(GLFW_KEY_L)) {
            orthoWidth = glm::max(orthoWidth - orthoChangeSpeed, 0.1f);
        }
