...
void Lab4::OnKeyPress(int key, int mods)
{
    ...
    const int moveStep = 10;  // pixels to move viewport
    const int scaleStep = 10; // pixels to scale viewport

    if (key == GLFW_KEY_I) {
        miniViewportArea.y += moveStep;  //sus
    }
    if (key == GLFW_KEY_K) {
        miniViewportArea.y -= moveStep;  //jos
    }
    if (key == GLFW_KEY_J) {
        miniViewportArea.x -= moveStep;  //stanga
    }
    if (key == GLFW_KEY_L) {
        miniViewportArea.x += moveStep;  //dreapta
    }
    if (key == GLFW_KEY_U) {
