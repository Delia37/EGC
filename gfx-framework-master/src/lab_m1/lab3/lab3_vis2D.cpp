...
void Lab3_Vis2D::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_W)) {
        logicSpace.y += translationSpeed * deltaTime;  //sus
    }
    if (window->KeyHold(GLFW_KEY_S)) {
        logicSpace.y -= translationSpeed * deltaTime;  //jos
    }
    if (window->KeyHold(GLFW_KEY_A)) {
        logicSpace.x -= translationSpeed * deltaTime;  //stanga
    }
    if (window->KeyHold(GLFW_KEY_D)) {
        logicSpace.x += translationSpeed * deltaTime;  //dreapta
    }
